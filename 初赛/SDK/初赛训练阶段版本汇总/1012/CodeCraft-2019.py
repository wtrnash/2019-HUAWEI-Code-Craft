...
def main():
    ...
    for index in range(len(cars)):
        direction = get_direction(cars[index].begin, cars[index].end)
        while (cars[index].plan_time, cars[index].begin, direction) in m.keys():
            if m[(cars[index].plan_time, cars[index].begin, direction)] >= 3:
                cars[index].plan_time += 1
            else:
                break

        m[(cars[index].plan_time, cars[index].begin, direction)] += 1

    cars = sorted(cars, key=sort_car)

    plan_time_map = defaultdict(int)
    last_time = 0
    cars_num = len(cars)
    count = 0
    for car in cars:
        count += 1
        last_time = max(last_time, car.plan_time)
        if plan_time_map[last_time] > 29 - (count / cars_num) * 6:
            last_time += 1

        plan_time_map[last_time] += 1
        car.plan_time = last_time
