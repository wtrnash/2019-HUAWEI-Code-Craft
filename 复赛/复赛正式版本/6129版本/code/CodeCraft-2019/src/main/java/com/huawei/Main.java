...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0;i<carArr.size();i++){
            ...
            {
                count++;
                if(count / persetCount < maxStartTime)    //预置车辆发的时候也跟着发车
                {
                    while(m.containsKey(count / persetCount)){
                        count++;
                    }

                    startTime = Math.max(Integer.valueOf(carArr.get(i)[4]), count / persetCount);
                }
                else {      //预置车辆发完后
                    startTime = maxStartTime + (count -  persetCount * maxStartTime)/ Math.round(26 + ((float) i / priorityNumber) * 10);
                }
            ...
            else if(i == priorityNumber)
            {
                ...
                if(count / persetCount < maxStartTime)    //预置车辆发的时候也跟着发车
                {
                    ...
                    startTime = maxStartTime + (count -  persetCount * maxStartTime)/ 35;
                }
