        ...
        for(int i=0;i<carArr.size();i++){
            if(i < priorityNumber)
            {
                ...
                if(count / 5 < maxStartTime)    //预置车辆发的时候也跟着发车
                {
                    ...
                    startTime = maxStartTime + (count -  5 * maxStartTime)/ Math.round(35 - ((float) i / priorityNumber) * 10);
                }
            ...
            {
                count++;
                startTime = commonStartTime + count / Math.round(40 - ((float) i / carArr.size()) * 10);   //普通车
            }
