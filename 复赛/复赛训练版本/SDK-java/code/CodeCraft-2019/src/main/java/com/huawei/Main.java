    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0;i<carArr.size();i++){
            if(i < priorityNumber)
            {
                ...
                {
                    while(m.containsKey(count / 5)){
                        count++;
                    }

                    startTime = Math.max(Integer.valueOf(carArr.get(i)[4]), count / 5);
                }
                else {      //预置车辆发完后
                    startTime = maxStartTime + Math.floorDiv((count -  5 * maxStartTime)/ Math.round(80 + ((float) i / priorityNumber) * 10), 5);
                }
            ...
            else if(i == priorityNumber)
            {
                ...
                if(count / 5 < maxStartTime)    //预置车辆发的时候也跟着发车
                {
                    ...
                    startTime = maxStartTime + (count -  5 * maxStartTime)/ 35;
                }
            ...
            {
                count++;
                startTime = commonStartTime + Math.floorDiv((count -  5 * maxStartTime)/ Math.round(100 + ((float) i / priorityNumber) * 10), 5);
            }
